...
class PaymentCheckLine(models.Model):
    ...
    def amount_to_words(self):
        for rec in self:
            rec.text_amount = rec.payment_id.currency_id.amount_to_text(rec.check_amount).replace('and', 'و') + " فقط لا غير"
    ...
    def reset_holding(self):
        for rec in self:
            rec.state = 'holding'
            print("1111111111111111111111111111111111", rec.move_ids)
